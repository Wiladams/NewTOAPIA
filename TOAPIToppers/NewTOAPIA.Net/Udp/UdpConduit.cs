...
namespace NewTOAPIA.Net
{
    ...
    /// </summary>
    public class UdpConduit : Observable<BufferChunk>
    {
        ...
        #region Constructors
        public UdpConduit(IPAddress address, int port)
            :this(new IPEndPoint(address, port))
        {
        }

        public UdpConduit(IPEndPoint endPoint)
        {
            fEndpoint = endPoint;
        ...
        void SetupBufferChunks()
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                BufferChunk newChunk = new BufferChunk(UdpSocket.BEST_PACKET_SIZE);
