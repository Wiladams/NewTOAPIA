...
namespace NewTOAPIA.Net.Rtp
{
    ...
    {
        SR = 200,   // Sender Report        stats on the Rtp packets I have sent
        RR = 201,   // Receiver Report      stats on the Rtp packets I have received
        SDES = 202, // Source Description   associated properties to an RtpStream
        BYE = 203,  // Bye                  membership management
    ...
    /// -------------------------------------------------------------------------------------------
    /// Each instance of RtpSender or RtpListener sends its own set of Rtcp packets.  
    /// In the case of an RtpSender, it sends:
    ///     SDES packets to describe the RtpStream of data it's sending out, 
    ///     SR   packets with statistics on what is being sent, 
    ///     BYE  when the RtpStream is destroyed.  
    /// In the case of an RtpListener, it sends:
    ///     SDES packets describing who is listening, 
    ///     RR   packets with statistics on every RtpStream received by that RtpListener, 
    ///     BYE  packets when the RtpListener is destroyed.
    /// 
    ...
    internal class CompoundPacketBuilder : IEnumerable
    {
        ...
        private Queue compoundPackets;

        // Packets that can be added to the compound packet
        private Queue senderReportPackets;
        private Queue appPackets;

        // Reports that can be added to the compound packet, used to build sub-packets
        private Queue receiverReports;
        private Queue byeReports;
        private Stack sdesReports;
        ...
        {
            compoundPackets = new Queue();

            // Packets that can be added to the compound packet
            senderReportPackets = new Queue();
            appPackets = new Queue();

            // Reports that can be added to the compound packet, used to build sub-packets
            receiverReports = new Queue();
            byeReports = new Queue();
            sdesReports = new Stack();
        }
    ...
    public class CompoundPacket : IEnumerable
    {
        ...
        private const int size = UdpSocket.BEST_PACKET_SIZE;
        ...
        internal void ParseBuffer()
        {
            ...
            if ((pt != (byte)RtcpPacketType.SR) && (pt != (byte)RtcpPacketType.RR))
            {
                throw new CompoundPacketException(string.Format(CultureInfo.CurrentCulture,
                    Strings.FirstPacketMustBeSROrRR, (byte)RtcpPacketType.SR, (byte)RtcpPacketType.RR, pt));
            }
        }
    ...
    internal class RtcpHeader : ICloneable, IRtcpData
    {
        ...
        public short Length
        {
            ...
            set
            {
                if (value < 0 || value > (UdpSocket.BEST_PACKET_SIZE / 4) - 1) // 32 bit words, minus header
                {
                    throw new RtcpHeaderException(string.Format(CultureInfo.CurrentCulture, Strings.InvalidLength,
                        value, (UdpSocket.BEST_PACKET_SIZE / 4) - 1));
                }
    ...
    public class RtcpPacket : IRtcpData
    {
        ...
        protected static void ValidatePacketType(byte expected, byte actual)
        {
            if(actual != expected)
            {
                throw new RtcpPacketException(string.Format(CultureInfo.CurrentCulture,
                    Strings.ActualPacketsDifferFromExpected, (RtcpPacketType)expected, expected, actual));
            }
        }
        ...
        /// </summary>
        protected RtcpPacket(RtcpPacketType type)
        {
        ...
        /// 
        /// This is meant to be overridden in derived classes, but can't be marked abstract
        /// unless the whole class is marked abstract, which is not desired.
        /// </summary>
    ...
    internal class SdesPacket : RtcpPacket, IRtcpData
    {
        ...
        {
            return (Queue)sdesReports.Clone();
        }
        ...
        public byte[] Data
        {
            ...
            set
            {
                ...
                if(value != null)
                {
                    ...
                    if (value.Length > UdpSocket.BEST_PACKET_SIZE - RtcpHeader.SIZE - Rtp.SSRC_SIZE - NAME_SIZE)
                    {
                        throw new ArgumentOutOfRangeException(string.Format(CultureInfo.CurrentCulture,
                            Strings.DataLengthIncorrect, UdpSocket.BEST_PACKET_SIZE, RtcpHeader.SIZE, Rtp.SSRC_SIZE, 
                            NAME_SIZE));
                    }
    ...
    internal class SrPacket : RtcpPacket
    {
        ...
        public SrPacket() : base(RtcpPacketType.SR) { }

        public SrPacket(uint ssrc)
            : base(RtcpPacketType.SR)
        {
            SSRC = ssrc;
        }
