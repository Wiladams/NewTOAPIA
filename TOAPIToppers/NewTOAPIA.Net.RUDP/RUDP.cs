...
public const int Hangupgen	= 0xffffffff;	/* used only in hangup messages */
...
//int	reliput(Conv*, Block*, byte*, ushort);
//Reliable *relstate(Rudpcb*, byte*, ushort, char*);
//void	relput(Reliable*);
//void	relforget(Conv *, byte*, int, int);
...
rudpkick(void *x)
{
	...
	Udphdr *uh;
	ushort rport;
	byte laddr[IPaddrlen], raddr[IPaddrlen];
	Block *bp;
	Rudpcb *ucb;
	Rudphdr *rh;
	Reliable *r;
...
rudpiput(Proto *rudp, Ipifc *ifc, Block *bp)
{
	...
	Conv *c;
	Rudpcb *ucb;
	byte raddr[IPaddrlen], laddr[IPaddrlen];
	ushort rport, lport;
	Rudppriv *upriv;
	Fs *f;
	byte *p;
...
{
	Udphdr *h;
	byte source[IPaddrlen], dest[IPaddrlen];
 ...
 */
Reliable*
relstate(Rudpcb *ucb, byte *addr, ushort port, char *from)
{
	Reliable *r, **l;
 ...
 */
void
relforget(Conv *c, byte *ip, int port, int originator)
{
	Rudpcb *ucb;
	Reliable *r, **l;
 ...
 */
int
reliput(Conv *c, Block *bp, byte *addr, ushort port)
{
	Block *nbp;
	Rudpcb *ucb;
	Rudppriv *upriv;
