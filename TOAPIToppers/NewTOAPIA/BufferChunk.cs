...
namespace NewTOAPIA
{
    using System.Text;  // For string conversions
    ...
    /// 
    /// When combined with NewTOAPIA.Net.Sockets.TSocket, you get a fully functional Socket class that
    ...
    public class BufferChunk: IDisposable, ICloneable
    {
        ...
        /// </summary>
        private int ValidDataStartIndex;      // The CLR should initialize this to zero

        /// <summary>
        /// Length of the valid data
        /// </summary>
        private int length;     // The CLR should initialize this to zero

        /// <summary>
        /// Flag indicating whether the object is disposed or not
        /// </summary>
        private bool disposed;  // The CLR should initialize this to false
        ...
        public BufferChunk(byte[] buffer, int index, int length)
        {
            ...
            this.buffer = buffer;
            this.ValidDataStartIndex = index;
            this.length = length;
        }
        #endregion
        ...
        public int Length
        {
            ...
            set
            {
                ...
                ValidatePointerData(ValidDataStartIndex, value, buffer.Length);
        ...
        private int AvailableBuffer
        {
            get
            {
                return buffer.Length - ValidDataStartIndex - length;
            }
        }
        ...
        public static explicit operator byte[](BufferChunk source)
        {
            ...
            Array.Copy(source.buffer, source.ValidDataStartIndex, returnBuffer, 0, source.length);
            return(returnBuffer);
        }
        /// <summary>
        ...
        public static BufferChunk operator+ (BufferChunk destination, BufferChunk source)
        {
            ...
            Array.Copy(source.buffer, source.ValidDataStartIndex, destination.buffer, destination.ValidDataStartIndex + destination.length, source.length);
        ...
        public static BufferChunk operator+ (BufferChunk destination, byte[] source)
        {
            ...
            Array.Copy(source, 0, destination.buffer, destination.ValidDataStartIndex + destination.length, source.Length);
            destination.length += source.Length;
        ...
        public static BufferChunk operator +(BufferChunk destination, float data)
        {
            ...
            // Advance the length 4, and set the data
            destination.length += 4;
            destination._SetFloat(destination.length - 4, data);
        ...
        public static BufferChunk operator +(BufferChunk destination, double data)
        {
            unsafe
            {
                long aLong = *(long*)&data;              // Read bits as long.
                return destination += (Int64)aLong;
            }
        }
        ...
        public byte this [int index]
        {
            get
            {
                ...
                return buffer[this.ValidDataStartIndex + index];
            }
            set
            {
                ValidateNonNegative(index);
                ...
                buffer[this.ValidDataStartIndex + index] = value;
            }
        }
        #endregion
        ...
        public object Clone()
        {
            return new BufferChunk(buffer, ValidDataStartIndex, length);
        }
        #endregion
        ...
        public void CopyTo(BufferChunk destination, int index)
        {
            ...
            Array.Copy(this.buffer, this.ValidDataStartIndex, destination.buffer, destination.ValidDataStartIndex + index, this.length);
        }

        public void CopyTo(IntPtr destination, Int32 length)
        {
            ValidateIntPtr(destination);
            ValidateNonNegative(length);
            ValidateNotZeroLength(length);
            ValidateSufficientData(length, this.length);

            Marshal.Copy(buffer, this.ValidDataStartIndex, destination, length);
            
            this.length -= length;
            this.ValidDataStartIndex += length;
        }

        public void CopyFrom(IntPtr source, Int32 alength)
        {
            ValidateIntPtr(source);
            ValidateNonNegative(alength);
            ValidateNotZeroLength(alength);
            ValidateSufficientSpace(alength, AvailableBuffer);

            Marshal.Copy(source, buffer, this.ValidDataStartIndex + this.length, alength);
            this.length += alength;
        }
        ...
        public void Clear()
        {
            for (int i = ValidDataStartIndex; i < ValidDataStartIndex + length; i++)
                buffer[i] = 0;
        }
        ...
        {
            return new BufferChunk(this.buffer, this.ValidDataStartIndex + index, length);
        }
        ...
        public UInt16 GetPaddedUInt16(int index)
        {
            ...
            else // The rest of the data in the buffer
            {
                ...
                while(index + offset < length)
                {
                    ...
                    ret += (UInt16)(buffer[this.ValidDataStartIndex + index + offset] << shift * 8);
                    offset++;
                }
            }
        ...
        public UInt32 GetPaddedUInt32(int index)
        {
            ...
            else // The rest of the data in the buffer
            {
                ...
                while(index + offset < length)
                {
                    ...
                    ret += ((UInt32)buffer[this.ValidDataStartIndex + index + offset]) << shift * 8;
                    offset++;
                }
            }
        ...
        public UInt64 GetPaddedUInt64(int index)
        {
            ...
            {
                int indexOffset = this.ValidDataStartIndex + index;
        ...
        public string GetUTF8String(int index, int length)
        {
            ...
            lock(utf8)
            {
                return utf8.GetString(buffer, this.ValidDataStartIndex + index, length);
            }
        }
        ...
        private Int16 _GetInt16(int index)
        {
            ...
            if(littleEndian)
            {
                ret  = (Int16)(buffer[this.ValidDataStartIndex + index + 0] << 1 * 8);
                ret += (Int16)(buffer[this.ValidDataStartIndex + index + 1] << 0 * 8);
            }
            else // BigEndian network -> BigEndian architecture
            {
                ret  = (Int16)(buffer[this.ValidDataStartIndex + index + 0] << 0 * 8);
                ret += (Int16)(buffer[this.ValidDataStartIndex + index + 1] << 1 * 8);
            }
        ...
        private Int32 _GetInt32(int index)
        {
            ...
            if(littleEndian)
            {
                ret  = buffer[this.ValidDataStartIndex + index + 0] << 3 * 8;
                ret += buffer[this.ValidDataStartIndex + index + 1] << 2 * 8;
                ret += buffer[this.ValidDataStartIndex + index + 2] << 1 * 8;
                ret += buffer[this.ValidDataStartIndex + index + 3] << 0 * 8;
            }
            else // BigEndian network -> BigEndian architecture
            {
                ret  = buffer[this.ValidDataStartIndex + index + 0] << 0 * 8;
                ret += buffer[this.ValidDataStartIndex + index + 1] << 1 * 8;
                ret += buffer[this.ValidDataStartIndex + index + 2] << 2 * 8;
                ret += buffer[this.ValidDataStartIndex + index + 3] << 3 * 8;
            }
        ...
        private unsafe UInt64 _GetUInt64(int index)
        {
            ...
            fixed(byte* pb = &buffer[this.ValidDataStartIndex + index])
            {
                ret = *((UInt64*)pb);
            }
        ...
        public void SetPaddedUInt16(int index, UInt16 data)
        {
            ...
            else // The rest of the data in the buffer
            {
                ...
                while(index + offset < length)
                {
                    ...
                    buffer[this.ValidDataStartIndex + index + offset] = (byte)(data >> (shift * 8));
                    offset++;
                }
            }
        }
        ...
        private void _SetInt16(int index, Int16 data)
        {
            ...
            if(littleEndian)
            {
                buffer[this.ValidDataStartIndex + index + 0] = (byte)(data >> (1 * 8));
                buffer[this.ValidDataStartIndex + index + 1] = (byte)(data >> (0 * 8));
            }
            else // BigEndian architecture -> BigEndian network
            {
                buffer[this.ValidDataStartIndex + index + 0] = (byte)(data >> (0 * 8));
                buffer[this.ValidDataStartIndex + index + 1] = (byte)(data >> (1 * 8));
            }
        }
        ...
        private void _SetInt32(int index, Int32 data)
        {
            ...
            if(littleEndian)
            {
                buffer[this.ValidDataStartIndex + index + 0] = (byte)(data >> (3 * 8));
                buffer[this.ValidDataStartIndex + index + 1] = (byte)(data >> (2 * 8));
                buffer[this.ValidDataStartIndex + index + 2] = (byte)(data >> (1 * 8));
                buffer[this.ValidDataStartIndex + index + 3] = (byte)(data >> (0 * 8));
            }
            else // BigEndian architecture -> BigEndian network
            {
                buffer[this.ValidDataStartIndex + index + 0] = (byte)(data >> (0 * 8));
                buffer[this.ValidDataStartIndex + index + 1] = (byte)(data >> (1 * 8));
                buffer[this.ValidDataStartIndex + index + 2] = (byte)(data >> (2 * 8));
                buffer[this.ValidDataStartIndex + index + 3] = (byte)(data >> (3 * 8));
            }
        }
        ...
        private unsafe void _SetFloat(int index, float data)
        {
            fixed (byte* pb = &buffer[this.ValidDataStartIndex + index])
            {
                *((float*)pb) = data;
