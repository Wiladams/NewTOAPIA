...
if not exist "%INETROOT%" (
  @echo Error: Could not make project root directory %INETROOT%.
  goto :EndOfScript
)
cd /d %INETROOT%
...
set ProjectIcon=%INETROOT%\codebox\tools\icon.ico
set "Args=/k set INETROOT=%INETROOT%&set CODEBOXPROJECT=%Project%"
set "Args=%Args%&\"%INETROOT%\codebox\tools\runme-sd.cmd\""
...
"%ToolsPath%shortcut" /p:cmd.exe /n:"%ShortcutName%.lnk" /i:"%ProjectIcon%" /a:"%Args%" /d:"%ShortcutStartup%" /o:0
