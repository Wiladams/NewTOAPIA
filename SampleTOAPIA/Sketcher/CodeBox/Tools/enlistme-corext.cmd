...
if not defined CODEBOX (
    if exist "%INETROOT%\build\icon.ico" (
        set BranchIcon=%INETROOT%\build\icon.ico
    ) else (
        set BranchIcon=%INETROOT%\tools\coretools\icon.ico
    )
) else (
    set BranchIcon=%INETROOT%\codebox\tools\icon.ico
)

set "Args=/k set INETROOT=%INETROOT%&set COREXTBRANCH=%Branch%"
if NOT "%COREXT_SHARED%"=="" (
...
set "Args=%Args%&%INETROOT%\tools\path1st\myenv.cmd"
if not defined CODEBOX (
...
%ToolsPath%shortcut /p:cmd.exe /n:"%ShortcutName%.lnk" /i:"%BranchIcon%" /a:"%Args%" /d:%ShortcutStartup% /o:0
